...
# Roll command parsing all args and suppress URL preview
def _compose_and_send(ctx, pool, modifier, image_url, notes, std, wild, expl, comp, total):
    ...
    tw, mh = sum(w), max(h)
    combo = Image.new('RGBA', (tw, mh), (0,0,0,0))
    x = 0
    for img in imgs:
        combo.paste(img, (x, 0)); x += img.width
...
async def roll_cmd(ctx, *args):
    ...
    # suppress the URL preview on trigger message
    try:
...
async def char_show(ctx, name: str):
    ...
    if not url:
        return await ctx.send(f"❌ No '{name}'")
    e = discord.Embed(title=name, color=discord.Color.blue())
